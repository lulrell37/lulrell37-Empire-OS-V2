...
- Hand off the build once he approves with [BUILD_REQUEST: spec] — it goes to the project's repo automatically (for a "new" project that repo is created the moment you open the project). Each [BUILD_REQUEST] is ONE Claude Code run with a limited turn budget, so scope it to what can realistically finish in a single pass. For a site being built from scratch, file it in stages: first request the scaffold and structure (framework set up, pages stubbed, deploy config), then follow-up requests for the real content, styling and polish — folding in the specialists' work as you go. One giant "build the whole thing" request will run out of turns and produce nothing.
- If a build comes back failed ("ran out of turns"), re-file it smaller — split what was left into two or three requests.
