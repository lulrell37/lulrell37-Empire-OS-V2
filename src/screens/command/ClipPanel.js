...
import{getClipJobs,updateClipJob,deleteClipJob}from '../../services/database';
  ...
  return(
    <View style={[s.wrap,{borderColor:ACCENT+'33'}]}>
      ...
      {!collapsed&&jobs.map(j=>{
        const inFlight=j.status==='queued'||j.status==='editing';
        const prog=inFlight?jobProgress('clip',j,hist):null;
        return(
        <View key={j.id} style={s.row}>
          <View style={s.rowTop}>
            ...
            <TaskRowActions what="clip edit" running={inFlight}
              onStop={async()=>{try{await cancelClipJob(j.issue_number);}catch{} await updateClipJob(j.id,{status:'cancelled'}); load();}}
              onDelete={async()=>{if(inFlight){try{await cancelClipJob(j.issue_number);}catch{}} await deleteClipJob(j.id); load();}}/>
          </View>
          {prog&&(
