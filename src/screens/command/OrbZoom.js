...
//   persona ring  ->  a persona's orb  ->  its memory web  ->  one region hub
// Zoom with the + / - buttons or (on web) the mouse wheel. Opens on the ring.
//
// Built on plain React Native Animated + PanResponder — no reanimated worklets,
// which is what hard-crashed the previous 3D version.
import React,{useState,useEffect,useMemo,useCallback,useRef}from 'react';
import{View,Text,StyleSheet,TouchableOpacity,ActivityIndicator,Dimensions,Platform,Animated,PanResponder,Image}from 'react-native';
...
// sin/cos lookup covering ±12 turns so the ring never runs out of range.
const SAMP=[],SIN=[],COS=[];
...
export default function OrbZoom({personaId,color,active,vizRef,personaPics={},onPickPersona,onLaunchGroup}){
  ...
  const[level,setLevel]=useState('group');   // open on the ring
  ...
  return(
    <View ref={wrapRef} style={s.wrap}>
      <Animated.View style={[{flex:1},{opacity:fade}]}>
        {level==='group'&&(
          <PersonaRing activeId={personaId} pics={personaPics} onPick={pick} onLaunch={launch}/>
        )}
        {level==='orb'&&(
      ...
      {level==='orb'&&<Text style={[s.hint,{color}]} pointerEvents="none">◈ + FOR MEMORY · − FOR ALL PERSONAS</Text>}
...
// --- The spinnable persona ring ----------------------------------------------

function PersonaRing({activeId,pics,onPick,onLaunch}){
  ...
  const pan=useMemo(()=>PanResponder.create({
    ...
    onPanResponderGrant:()=>{rot.stopAnimation(v=>{rotStart.current=v;});},
    onPanResponderMove:(_,g)=>{rot.setValue(rotStart.current-g.dx*0.010);},
    onPanResponderRelease:(_,g)=>{
      Animated.decay(rot,{velocity:-g.vx*0.010,deceleration:0.997,useNativeDriver:false}).start();
    },
  }),[]);// eslint-disable-line react-hooks/exhaustive-deps

  const rx=Math.min(size.w,520)*0.34;
  const orbs=useMemo(()=>PERSONA_LIST.map((p,i)=>{
    const off=(i/PERSONA_LIST.length)*2*Math.PI;
    const ang=Animated.add(rot,off);
    const sinA=ang.interpolate({inputRange:SAMP,outputRange:SIN,extrapolate:'clamp'});
    const cosA=ang.interpolate({inputRange:SAMP,outputRange:COS,extrapolate:'clamp'});
    return{
      p,
      translateX:Animated.multiply(sinA,rx),
      translateY:Animated.multiply(cosA,10),
      scale:cosA.interpolate({inputRange:[-1,1],outputRange:[0.5,1.12]}),
      opacity:cosA.interpolate({inputRange:[-1,1],outputRange:[0.25,1]}),
    };
  }),[rot,rx]);
  ...
  return(
    <View style={{flex:1}} onLayout={e=>{const{width,height}=e.nativeEvent.layout;setSize({w:width,h:height});}}>
      <View style={StyleSheet.absoluteFill} {...pan.panHandlers}>
        {orbs.map(({p,translateX,translateY,scale,opacity})=>(
          <Animated.View key={p.id} style={[s.ringOrbWrap,{opacity,transform:[{translateX},{translateY},{scale}]}]}>
            <TouchableOpacity activeOpacity={0.8} delayLongPress={280}
              onPress={()=>onPick(p.id)} onLongPress={()=>toggle(p.id)}
              style={[s.ringOrb,{borderColor:p.color,backgroundColor:p.color+(group.includes(p.id)?'33':p.id===activeId?'26':'14')}]}>
              {pics[p.id]
                ?<Image source={{uri:pics[p.id]}} style={s.ringOrbImg}/>
                :<Text style={[s.ringOrbIcon,{color:p.color}]}>{p.icon}</Text>}
            </TouchableOpacity>
          </Animated.View>
        ))}
      </View>
...
const s=StyleSheet.create({
  ...
  ringOrbWrap:{position:'absolute',left:'50%',top:'42%',marginLeft:-32,marginTop:-32,width:64,height:64},
  ringOrb:{width:64,height:64,borderRadius:32,borderWidth:1.5,alignItems:'center',justifyContent:'center',overflow:'hidden'},
  ringOrbImg:{width:'100%',height:'100%'},
  ringOrbIcon:{fontFamily:'monospace',fontSize:15,fontWeight:'700'},
