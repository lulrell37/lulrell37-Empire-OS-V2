          ...
          <View key={l.id} style={s.row}>
            <TouchableOpacity activeOpacity={0.7} onPress={()=>setOpen(isOpen?null:l.id)}>
              <View style={s.rowTop}>
                ...
                {l.heat>0&&<Text style={[s.heat,{color:l.heat>=70?'#E86A4B':l.heat>=45?'#D9A441':'#6a6250'}]}>♦{l.heat}</Text>}
              ...
              {!!l.signal&&!isOpen&&<Text style={s.signal} numberOfLines={1}>◈ {l.signal}</Text>}
            ...
            {isOpen&&(
              <View style={s.detail}>
                ...
                {!!l.signal&&<Text style={s.dK}>sig   <Text style={s.dV}>{l.signal}</Text></Text>}
...
const s=StyleSheet.create({
  ...
  heat:{fontFamily:'monospace',fontSize:8,fontWeight:'700'},
  ...
  signal:{fontFamily:'monospace',fontSize:8,color:'#9a8f6a',marginTop:1},
