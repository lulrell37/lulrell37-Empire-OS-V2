...
// around a soft glowing core, in the persona's colour. At rest it breathes,
// shimmers and throws the odd sparkle; while the persona speaks the whole cloud
// blooms outward with a travelling shockwave, then settles.
//
  ...
  async function onContextCreate(gl){
    try{
      ...
      const pointsMat=new THREE.ShaderMaterial({
        ...
        vertexShader:`
          ...
          void main(){
            ...
            // idle drift + speaking amplitude
            float disp=sin(uTime*aFreq+aPhase)*(0.05+uAmp*0.32);
            // travelling shockwave while speaking — a ring sweeping pole to pole
            disp+=uPulse*0.30*sin(dir.y*5.0-uPulse*14.0+aPhase*0.3);
            float bloom=1.0+uAmp*0.22+uLit*0.18+uPulse*0.30;
            vec3 p=dir*(1.0+disp)*bloom;
            vec4 mv=modelViewMatrix*vec4(p,1.0);
            gl_Position=projectionMatrix*mv;
            // sparse per-point sparkle even at rest
      ...
      const animate=()=>{
        ...
        // rising edge of "speaking" fires a one-shot bloom
        if(speaking&&!engine.prevSpeaking)engine.pulse=1;
        engine.prevSpeaking=speaking;
        engine.pulse=Math.max(0,engine.pulse-dt*1.6); // ~0.6s decay
